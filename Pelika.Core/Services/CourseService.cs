...
using Pelika.DataLayer.Migrations;
...
namespace Pelika.Core.Services
{
    public class CourseService : ICourseService
    {
        ...
        public CourseService(PelikaContext context)
        {
            _context = context;
        }
            ...
            int startPrice = 0, int endPrice = 0, List<int> selectedGroups = null, int take = 0)
        {
            ...
            switch (orderByType)
            {
                ...
                case "updatedate":
                {
                    result = result.OrderByDescending(c => c.UpdatedDate);
                    break;
                }
            }
        ...
        public Course GetCourseForShow(int courseId)
        {
            return _context.Courses.Include(c => c.CourseEpisodes).Include(c => c.CourseStatus)
                .Include(c => c.CourseLevel).Include(c => c.User).Include(c=>c.UserCourses).FirstOrDefault(c => c.CourseId == courseId);
        }
        ...
        {
            return _context.Courses.Sum(c => c.CourseId-(c.CourseId-1) );
        }
    }
}
